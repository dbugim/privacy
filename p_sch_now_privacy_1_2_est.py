...
def log_error_to_file(error_message, log_file_path="G:\\Meu Drive\\Privacy_free\\p_sch_privacy_free_error_logs.txt", traceback_info=None):
    """
    Logs an error message to a file with a timestamp, optionally including traceback.
    Creates the directory if it does not exist.
    """
    timestamp = datetime.now().strftime("%m/%d/%Y at %H:%M") # MM/DD/YYYY at HH:MM
    log_entry = f"Error on {timestamp}: {error_message}\n"
...
def open_chrome_in_privacy_login_page():
    ...
    try:
        ...
        print(f"❌ {error_message}")
        log_error_to_file(error_message, traceback_info=full_traceback)
...
def mark_caption_as_used(caption):
    ...
    def cleanup(pw, context, browser_process, page):
        ...
        try:
            ...
            full_traceback = traceback.format_exc()
            print(f"❌ {error_msg}")
            log_error_to_file(error_msg, traceback_info=full_traceback)
...
def main():
    ...
    try:
        ...
        full_traceback = traceback.format_exc()
        print(error_msg)
        log_error_to_file(error_msg, traceback_info=full_traceback)  # Log the error with traceback
        cleanup(pw, context, browser_process) # Aqui page pode ser None, mas cleanup já lida com isso
    ...
    try:
        ...
        try:
            ...
            warning_msg = "⚠️ Warning: pyautogui not installed, skipping fullscreen."
            full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
            print(warning_msg)
            log_error_to_file(warning_msg, traceback_info=full_traceback)  # Log the warning with traceback
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                specific_error_msg = f"❌ Failed to insert username on attempt {attempt + 1}: {e}"
                full_traceback = traceback.format_exc()
                print(f"❌ {specific_error_msg}")
                log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                if attempt < max_retries - 1:
                    time.sleep(2)
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                specific_error_msg = f"❌ Failed to insert password on attempt {attempt + 1}: {e}"
                full_traceback = traceback.format_exc()
                print(f"❌ {specific_error_msg}")
                log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                if attempt < max_retries - 1:
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                specific_error_msg = f"❌ Failed to click Entrar button on attempt {attempt + 1}: {e}"
                full_traceback = traceback.format_exc()
                print(f"❌ {specific_error_msg}")
                log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                if attempt < max_retries - 1:
        ...
        print("Checking if login screen still persists...")
        ...
        try:
            if insert_username(page) and insert_password(page) and click_on_entrar_button(page):
                print("Login elements still available, trying to execute LOGIN OPERATION flow again.")
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        specific_error_msg = f"❌ Failed to insert username on retry attempt {attempt + 1}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        specific_error_msg = f"❌ Failed to insert password on retry attempt {attempt + 1}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        specific_error_msg = f"❌ Failed to click Entrar button on retry attempt {attempt + 1}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                ...
                print("Checking if login screen still persists after retry...")
                try:
                    if insert_username(page) and insert_password(page) and click_on_entrar_button(page):
                        error_msg = "Unable to log in to Privacy VIP, aborting process to avoid account locking."
                        full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                        print(error_msg)
                        log_error_to_file(error_msg, traceback_info=full_traceback)
                    else:
                    ...
                    error_msg = f"Error during post-retry login check or navigation: {e}"
                    full_traceback = traceback.format_exc()
                    print(f"❌ {error_msg}")
                    log_error_to_file(error_msg, traceback_info=full_traceback)
            else:
                # Navigate to Mural page after successful login
                print("\n✅ Navigating to Mural page after successful login...")
                page.goto("https://privacy.com.br/board")
                time.sleep(5)
                print(f"✅ Navigated to: {page.url}")
        except Exception as e:
            error_msg = f"Error during initial login check or navigation: {e}"
            full_traceback = traceback.format_exc()
            print(f"❌ {error_msg}")
            log_error_to_file(error_msg, traceback_info=full_traceback)
        ...
        try:
            ...
            error_msg = f"Error while trying to close pop-up: {e}"
            full_traceback = traceback.format_exc()
            print(f"❌ {error_msg}")
            log_error_to_file(error_msg, traceback_info=full_traceback)
        # endregion
        ...
        # region Operation to handle time
        now = datetime.now()

        # Check hour value
        current_hour = now.hour

        # Calculate manual_iterations (24 - current_hour)
        manual_iterations = 24 - current_hour

        print(f"Current time: {now.strftime('%H:%M')}")
        print(f"Hours remaining (iterations): {manual_iterations}")
        # endregion

        # Loop for increment hours
        for i in range(manual_iterations):
            # Calculate the actual hour based on current_hour + current loop index
            hora = current_hour + i

            # Now hora is defined and can be formatted
            hora_str = f"{hora:02d}"

            print(f"\n=== PROCESSING HOUR {hora_str}:00 (Loop {i + 1}/{manual_iterations}) ===")
            ...
            for minute_str, click_func in minute_functions.items():
                ...
                for attempt in range(max_retries):
                    ...
                    try:
                        ...
                        specific_error_msg = f"❌ Failed to click Entrar button during minute loop on attempt {attempt + 1}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                ...
                # Even if the loop above finishes without success,
                # the code will now naturally move to the Postar section.
                # endregion
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        specific_error_msg = f"Error in click_on_postar_btn on attempt {attempt + 1}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                    ...
                    error_msg = f"❌ Failed to click Postar button after all attempts for minute {minute_str}."
                    full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                    print(error_msg)
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                ...
                # region Try to click the Feed button with retries
                for attempt in range(max_retries):
                    try:
                        ...
                        specific_error_msg = f"Error clicking Feed button on attempt {attempt + 1} for minute {minute_str}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                    ...
                    error_msg = f"CRITICAL: Failed Feed at {minute_str}. Refreshing and skipping..."
                    full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                    print(error_msg)
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                if not available_media:
                    warning_msg = f"WARNING: No media available for {minute_str}. Skipping..."
                    full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                    print(warning_msg)
                    log_error_to_file(warning_msg, traceback_info=full_traceback)
                    continue
                ...
                try:
                    if click_to_send_file_url(page, current_media_filename):
                        ...
                        error_msg = f"CRITICAL: Failed Direct Upload at {minute_str}. Refreshing and skipping..."
                        full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                        print(error_msg)
                        log_error_to_file(error_msg, traceback_info=full_traceback)
                        page.reload()
                    ...
                    error_msg = f"CRITICAL: Exception during Direct Upload at {minute_str}: {e}. Refreshing and skipping..."
                    full_traceback = traceback.format_exc()
                    print(f"❌ {error_msg}")
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        specific_error_msg = f"Error clicking Text Area on attempt {attempt + 1} for minute {minute_str}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                    ...
                    error_msg = f"CRITICAL: Failed Text Area at {minute_str}. Refreshing and skipping..."
                    full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                    print(error_msg)
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                try:
                    ...
                    error_msg = f"Error pasting caption for minute {minute_str}: {e}"
                    full_traceback = traceback.format_exc()
                    print(f"❌ {error_msg}")
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                try:
                    ...
                    error_msg = f"CRITICAL: Failed to click Post 24 horas at {minute_str}: {e}. Refreshing and skipping..."
                    full_traceback = traceback.format_exc()
                    print(f"❌ {error_msg}")
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        specific_error_msg = f"Error clicking Schedule switch on attempt {attempt + 1} for minute {minute_str}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                    ...
                    error_msg = f"❌ Failed to click the Schedule switch after all attempts for minute {minute_str}."
                    full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                    print(error_msg)
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        specific_error_msg = f"Error clicking Time element on attempt {attempt + 1} for minute {minute_str}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                    ...
                    error_msg = f"CRITICAL: Could not open time picker at {minute_str}. Refreshing..."
                    full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                    print(error_msg)
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                # --- NEW LOGIC: CLICK HOUR UP BASED ON CURRENT HOUR LOOP ---
                # i comes from the OUTER 'for i in range(manual_iterations)'
                clicks_to_perform = i + 1

                print(f"[{minute_str}] Adjusting Hour: Clicking 'Up' {clicks_to_perform} time(s)")

                try:
                    for click_num in range(clicks_to_perform):
                        click_hour_up(page)
                        time.sleep(0.5)  # Brief pause so the UI registers each click
                except Exception as e:
                    error_msg = f"Error adjusting hour for minute {minute_str}: {e}"
                    full_traceback = traceback.format_exc()
                    print(f"❌ {error_msg}")
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                # Now continue to your minute selection
                try:
                    if click_func(page):
                        print(f"[{minute_str}] Minute {minute_str} selected.")
                except Exception as e:
                    error_msg = f"Error selecting minute via function for minute {minute_str}: {e}"
                    full_traceback = traceback.format_exc()
                    print(f"❌ {error_msg}")
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        specific_error_msg = f"Error clicking minute selection on attempt {attempt + 1} for minute {minute_str}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                    ...
                    error_msg = f"CRITICAL: Failed to select minute at {minute_str}. Refreshing..."
                    full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                    print(error_msg)
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                for attempt in range(max_retries):
                    ...
                    try:
                        ...
                        specific_error_msg = f"Error selecting minute via dictionary/locator on attempt {attempt + 1} for minute {minute_str}: {e}"
                        full_traceback = traceback.format_exc()
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                    ...
                    error_msg = f"CRITICAL: All attempts for minute {minute_str} failed. Refreshing and skipping..."
                    full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                    print(error_msg)
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        specific_error_msg = f"Error clicking Minute Up button on attempt {attempt + 1} for minute {minute_str}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                    ...
                    error_msg = f"CRITICAL: Failed Minute Up at {minute_str}. Refreshing..."
                    full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                    print(error_msg)
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                for attempt in range(max_retries):
                    try:
                        if click_aplicar_in_modal(page):
                            break
                        else:
                            print(f"[{minute_str}] Aplicar Attempt {attempt + 1} failed.")
                            if attempt < max_retries - 1:
                                time.sleep(2)
                    except Exception as e:
                        specific_error_msg = f"Error clicking Aplicar button on attempt {attempt + 1} for minute {minute_str}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                    ...
                    error_msg = f"CRITICAL: Failed Aplicar at {minute_str}. Refreshing..."
                    full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                    print(error_msg)
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        specific_error_msg = f"Error clicking Avançar button on attempt {attempt + 1} for minute {minute_str}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                    ...
                    error_msg = f"CRITICAL: Failed Avançar at {minute_str}. Refreshing..."
                    full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                    print(error_msg)
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        specific_error_msg = f"Error clicking Agendar button on attempt {attempt + 1} for minute {minute_str}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                    ...
                    error_msg = f"❌ CRITICAL: Failed Agendar at {minute_str}. Refreshing..."
                    full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                    print(error_msg)
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        specific_error_msg = f"Error clicking Concluído button on attempt {attempt + 1} for minute {minute_str}: {e}"
                        full_traceback = traceback.format_exc()
                        print(f"❌ {specific_error_msg}")
                        log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                        if attempt < max_retries - 1:
                    ...
                    error_msg = f"CRITICAL: Failed Concluído at {minute_str}. Refreshing..."
                    full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
                    print(error_msg)
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
                ...
                media_idx += 1  # Media file counter
                j += 1          # Phrase counter
                k += 1          # General counter
                ...
                try:
                    ...
                    error_msg = f"Error marking caption or media as used for minute {minute_str}: {e}"
                    full_traceback = traceback.format_exc()
                    print(f"❌ {error_msg}")
                    log_error_to_file(error_msg, traceback_info=full_traceback)
                    page.reload()
        ...
        try:
            ...
            error_msg = f"Error scrolling to top of page: {e}"
            full_traceback = traceback.format_exc()
            print(f"❌ {error_msg}")
            log_error_to_file(error_msg, traceback_info=full_traceback)
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                specific_error_msg = f"Error clicking Menu button on attempt {attempt + 1}: {e}"
                full_traceback = traceback.format_exc()
                print(f"❌ {specific_error_msg}")
                log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                if attempt < max_retries - 1:
            ...
            error_msg = f"❌ Failed to click Menu button after all attempts."
            full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
            print(error_msg)
            log_error_to_file(error_msg, traceback_info=full_traceback)
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                specific_error_msg = f"Error clicking Sair button on attempt {attempt + 1}: {e}"
                full_traceback = traceback.format_exc()
                print(f"❌ {specific_error_msg}")
                log_error_to_file(specific_error_msg, traceback_info=full_traceback)
                if attempt < max_retries - 1:
        ...
        if not sair_clicked:
            error_msg = f"❌ Failed to click on sair after all attempts."
            full_traceback = traceback.format_exc() if sys.exc_info()[0] else None
            print(error_msg)
            log_error_to_file(error_msg, traceback_info=full_traceback)
        ...
        error_msg = f"An unexpected error occurred during automation! Details: {e}"
        full_traceback = traceback.format_exc()
        print(f"❌ {error_msg}")
        log_error_to_file(error_msg, traceback_info=full_traceback)  # Log the unexpected error with traceback
        sys.exit(1)  # Exit with an error code
        ...
        try:
            ...
            full_traceback = traceback.format_exc()
            print(f"❌ {error_msg}")
            log_error_to_file(error_msg, traceback_info=full_traceback)  # Log cleanup errors
        ...
        try:
            ...
            print(f"❌ Failed to write end-of-iteration log separator: {file_write_err}")
            # No log_error_to_file here to avoid infinite recursion if log_error_to_file itself fails
