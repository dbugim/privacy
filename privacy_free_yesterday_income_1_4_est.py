...
def confirm_download_and_rebuild_report(page, click_on_confirmar_button):
    ...
    try:
        ...
        # Sum all values in column C (3th column, 0-based index)
        commission_sum = df.iloc[:, 2].sum()
        ...
        if os.path.exists(target_excel):
            ...
            wb = Workbook()
            ws = wb.active
            # Adicione um cabeçalho inicial se for um novo arquivo para ter uma célula de referência
            ws.cell(row=1, column=2, value="Soma da Comissão")
            # Salve para que a célula de referência exista para formatação futura
            wb.save(target_excel)
            wb = load_workbook(target_excel)
            ws = wb.active

        # Find the last filled row in column C (coluna 3, 1-based in openpyxl)
        col_index = 3  # Coluna C (A=1, B=2, C=3, etc.)
        last_filled_row = 0
        ...
        if last_filled_row >= 1:
            ...
            # Se não houver célula acima para copiar, defina um formato padrão (ex: moeda)
        ...
        wb.save(target_excel)
        print(f"Sum {commission_sum} inserted into {target_excel} at row {next_row}, column B, with formatting preserved.")
