...
def confirm_download_and_rebuild_report(page, click_on_confirmar_button):
    ...
    try:
        ...
        # Ensure there are at least 3 columns (for column C, index 2)
        ...
        # Find the last filled row in column C (index 2)
        # Get non-NaN values in column 2
        col_c = target_df.iloc[:, 2]
        filled_rows = col_c[~col_c.isna()].index
        ...
        # Insert the sum into column C (index 2) at the next row
        target_df.iloc[next_row, 2] = commission_sum
        ...
        print(f"Sum {commission_sum} inserted into {target_excel} at row {next_row + 1}, column C.")
