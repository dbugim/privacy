...
def messages_found_counter(page):
    """
    Conta e imprime a quantidade de "room items" (mensagens/conversas) visíveis
    na lista de salas de chat e retorna esse número.
    """
    messages_found = 0
    try:
        # Seletor para a lista de salas (o contêiner pai)
        rooms_list_selector = "#rooms-list"

        # Seletor para cada item de sala dentro da lista
        room_item_selector = f"{rooms_list_selector} > div.vac-room-item"

        # Tenta encontrar o contêiner da lista de salas primeiro
        rooms_list_locator = page.locator(rooms_list_selector)
        if not rooms_list_locator.is_visible():
            print(f"O contêiner da lista de salas '{rooms_list_selector}' não está visível.")
            return 0

        # Encontra todos os itens de sala visíveis dentro do contêiner
        room_items = page.locator(room_item_selector)
        messages_found = room_items.count()

        if messages_found > 0:
            print(f"Foram encontradas {messages_found} mensagens/conversas na lista.")
        else:
            print("Nenhuma mensagem/conversa foi encontrada na lista.")

        return messages_found

    except Exception as e:
        print(f"Ocorreu um erro ao tentar contar as mensagens/conversas: {e}")
        return 0

def read_the_messages(page, num_messages=5):
    """
    Reads the last 'num_messages' from the chat and saves them to the global 'last_messages' variable.
    """
    global last_messages
    last_messages = [] # Clear previous messages before reading new ones

    try:
        # Selector for all message text spans within the messages container
        # This targets the <span> elements that directly contain the message text.
        message_text_selector = ".vac-messages-container .vac-message-card .vac-format-container span"

        # Try to locate all message text elements
        message_elements = page.locator(message_text_selector)

        if message_elements.count() == 0:
            print("No messages found with the specified selector.")
            return False

        # Get the text content of all messages
        all_texts = message_elements.all_text_contents()

        # Filter out any empty strings or non-message content that might be captured
        filtered_texts = [text.strip() for text in all_texts if text.strip()]

        # Get the last 'num_messages'
        messages_to_save = filtered_texts[-num_messages:]

        if messages_to_save:
            last_messages.extend(messages_to_save)
            print(f"Successfully read {len(messages_to_save)} messages.")
            # print(f"Last messages: {last_messages}") # Uncomment for debugging
            return True
        else:
            print("Could not extract any message text.")
            return False

    except Exception as e:
        print(f"Error in read_the_messages: {str(e)}")
        return False
...
def main():
    ...
    try:
        ...
        # region Tenta contar as mensagens encontradas com retries
        max_retries = 3
        messages_found = 0
        for attempt in range(max_retries):
            print(f"\nTentativa {attempt + 1} para contar as mensagens...")
            messages_found = messages_found_counter(page)
            if messages_found > 0:
                print(f"Contagem de mensagens bem-sucedida: {messages_found} mensagens encontradas!")
                break
            else:
                print(f"Tentativa {attempt + 1} falhou ou nenhuma mensagem foi encontrada.")
                if attempt < max_retries - 1:
                    print("Aguardando antes da próxima tentativa...")
                    page.wait_for_timeout(2000) # Espera por 2 segundos
        else:
            print("Falha ao contar as mensagens após todas as tentativas.")

        page.wait_for_timeout(3000) # Espera por 3 segundos antes de fechar
        # endregion Tenta contar as mensagens encontradas com retries


        # # region Try to click the unread messages button with retries
        # max_retries = 3
        # for attempt in range(max_retries):
        #     print(f"\nAttempt {attempt + 1} to click unread messages button...")
        #     if click_on_unread_messages(page):
        #         print("Successfully clicked unread messages button!")
        #         break
        #     else:
        #         print(f"Attempt {attempt + 1} failed or unread count was zero.")
        #         if attempt < max_retries - 1:
        #             print("Waiting before next attempt...")
        #             page.wait_for_timeout(2000) # Wait for 2 seconds
        # else:
        #     print("Failed to click unread messages button after all attempts, or unread count remained zero.")

        # page.wait_for_timeout(3000) # Wait for 3 seconds before closing
        # # endregion Try to click the unread messages button with retries

        

        # # region Try to read the last messages with retries
        # max_retries = 3
        # for attempt in range(max_retries):
        #     #print(f"\nAttempt {attempt + 1} to read messages...")
        #     if read_the_messages(page, num_messages=5): # You can change num_messages to 3 if preferred
        ...
        #             page.wait_for_timeout(2000)
        # else:
        #     print("Failed to read messages after all attempts.")

        # page.wait_for_timeout(3000)
        # # endregion Try to read the last messages with retries

        # print(f"Final collected messages: {last_messages}")
