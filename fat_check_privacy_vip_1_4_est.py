...
def read_report_and_delete():
    """
    Reads the most recent XLSX report, sums Column D, updates the 
    global privacy_vip_income, and deletes the file.
    """
    ...
    try:
        ...
        # 5. CLOSE the workbook before deleting (Critical for Windows)
        wb.close() 
        ...
        print(f"File {os.path.basename(latest_file)} deleted successfully.")
        return True

    except Exception as e:
        print(f"Error in read_report_and_delete: {str(e)}")
        return False
    
privacy_vip_income = 0.0

def main():
    global privacy_free_income
    pw = None
    context = None
    ...
    try:
        ...
        # region Ler o relatório baixado e calcular a renda
        read_report_and_delete()
        # endregion
